...
/**
 * CameraX/ML Kit text adapter for directed OCR. The live preview is rendered
 * ONLY inside the OCR strip (the strip is the camera eye the operator aims
 * with) — the caller clips this composable to the strip bounds, so the rest
 * of the screen stays pure black. Emits raw text blocks to the coordinator;
 * [DirectedOcr] plus explicit operator review decide what, if anything,
 * becomes a code. This engine NEVER submits.
 */
