...
class ScanCoordinator(
    private val onAccepted: (value: String, fromOcr: Boolean) -> Unit,
    ...
    fun onScanned(raw: String, fromOcr: Boolean) {
            ...
            when (val outcome = decision.evaluate(raw)) {
                is ScanOutcome.Accepted -> {
                    if (fromOcr) {
                        ...
                        if (best != null) onAccepted(best.token, true)
                        else onRejected("OCR_LOW_CONFIDENCE")
                    } else {
                        onAccepted(outcome.value, false)
                    }
                }
