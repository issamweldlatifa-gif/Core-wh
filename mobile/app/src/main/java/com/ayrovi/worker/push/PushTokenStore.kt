 ...
 */
class PushTokenStore(context: Context) : PushTokenStorage {
    ...
    override fun needsSync(): Boolean {
        val current = token() ?: return false
        return current != prefs.getString(KEY_SYNCED, null)
    }

    override fun markSynced(token: String) = prefs.edit().putString(KEY_SYNCED, token).apply()

    /** Logout: the token stays valid for the device but is no longer ours to send. */
    override fun clearSynced() = prefs.edit().remove(KEY_SYNCED).apply()
