...
class ScannerManager(
    ...
    @Synchronized fun capture(raw: String, source: ScanSource, symbology: ScanSymbology = ScanSymbology.UNKNOWN): ScanResult? {
        ...
        return when (val outcome = decision.evaluate(raw, clock())) {
            is ScanOutcome.Accepted -> ScanResult(outcome.value, source, symbology).also {
                mutable.value = ScannerState(ScannerStatus.CAPTURED, "Barcode captured — awaiting workflow validation", it)
            }
            is ScanOutcome.Rejected -> {
                mutable.value = when (outcome.reason) {
                    RejectReason.DUPLICATE, RejectReason.DEBOUNCED -> ScannerState(ScannerStatus.DUPLICATE, "Duplicate scan — not submitted")
                    RejectReason.EMPTY -> ScannerState(ScannerStatus.INVALID, "No barcode was read")
                    RejectReason.INVALID -> ScannerState(ScannerStatus.INVALID, "Invalid barcode input — scan again or enter the code")
                }
                null
            }
        }
    ...
    @Synchronized fun timeout() { if (enabled && mutable.value.status == ScannerStatus.SCANNING) mutable.value = ScannerState(ScannerStatus.TIMEOUT, "Scan timed out — retry or enter the code") }
    @Synchronized fun cancel() { if (enabled) mutable.value = ScannerState(ScannerStatus.CANCELLED, "Scan cancelled — nothing submitted") }
    @Synchronized fun unavailable(reason: String) { mutable.value = ScannerState(ScannerStatus.UNAVAILABLE, reason) }
}
