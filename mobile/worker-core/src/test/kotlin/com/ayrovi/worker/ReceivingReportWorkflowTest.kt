...
class ReceivingReportWorkflowTest {
    ...
    @Test fun `open failure is terminal and retry recovers the report`() = runTest {
        val back = backend()
        back.failure = WorkerRepository.ApiException(500, "backend down")
        val flow = workflow(back)
        flow.initialize(); runCurrent()
        // Terminal error state: loading is OFF (no endless spinner),
        // no report, and a worker-safe message for the RETRY branch.
        ...
        // RETRY (refresh with no session re-runs the whole open sequence).
        back.failure = null
        flow.refresh(); runCurrent()
        assertFalse(flow.state.value.loading)
        assertEquals("s1", flow.state.value.sessionId)
    ...
    @Test fun `refresh failure keeps the loaded report and clears loading`() = runTest {
        val back = backend()
        val flow = workflow(back)
        flow.initialize(); runCurrent()
        back.failure = WorkerRepository.ApiException(500, "backend down")
