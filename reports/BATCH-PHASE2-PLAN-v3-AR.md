...
**الحالة:** العقد + الخدمات + Admin UI + تطبيق العامل + **الاستلام (BATCH IN) — كلها مكتملة وCI أخضر** — الفرع `feature/ayrovi-batch` @ `fa52e0c`
...
2. ~~Admin UI~~ ✅ **مكتملة** (`6999a48`).
3. ~~تطبيق العامل~~ ✅ **مكتملة** (`00ca326`→`cf3c713`→`198c969`) — BatchGateway + BatchBuildWorkflow في worker-core (**تحقق JVM محلي kotlinc: 128/128 شامل 120 اختبار البيت**)؛ BatchScreen/ViewModel بنمط TempStorage حرفيًا؛ سطح ماسح واحد؛ طباعة ملصقات AYP عبر Android Print Framework (بلا SDK)؛ مهمة 'batch' في السجل **بلا محطة** (مطابق للأمر)؛ v69/1.8.0-batch.2؛ CI: backend 264 + android تجميع/اختبارات/محاكي كلها خضراء.
4. ~~الاستلام~~ ✅ **مكتملة** (`bb15928`→`fa52e0c`) — BATCH IN على الطرفية: قائمة الانتظار (SENT + IN_PROGRESS قابلة للاستكمال) → START → فحص AYP (**كل مسح = وحدة، الصدى no-op مؤكد من الخادم، العدادات حقيقة الخادم**) → إكمال عند 10/10 (مرآة عميل + إلزام خادم)؛ **فحص المحطة المنجز: DISPATCH صادر فقط — لا يصلح للاستلام الوارد؛ المهمة بلا محطة ولم تُنشأ محطة BATCH**؛ worker-core 136/136 محليًا (kotlinc)؛ v70/1.8.0-batch.3؛ CI أخضر كامل (266 backend + تجميع/محاكي).
3. ~~تطبيق العامل~~ (انظر أعلاه ✅) (إنشاء عميل → دفعة → إضافة وحدات AYP → تصدير/طباعة الملصقات → إرسال).
5. **الاستلام** (فحص AYP → 10/10 → completed → تقرير) — كل مسح = وحدة واحدة؛ SKU×10 = 10 عناصر (التكرار ليس تكرارًا). **+ التحقق الإلزامي: إعادة استخدام محطة DISPATCH بدل إنشاء محطة BATCH جديدة.**
6. **E2E + تقرير + Release** (فرنسا 10 منتجات/دفعة واحدة؛ تونس فتح→مسح→اكتمال؛ A+B بالتوازي؛ بيانات AYBTEST-/AYPTEST- قابلة للتصفير).

**قاعدة البوابة:** لا انتقال لشريحة جديدة باختبارات حمراء — الوضع الآن أخضر بالكامل على الفرع. **التالي: E2E + Release** (سيناريو فرنسا 10 منتجات→10 وحدات→دفعة→قبول→طباعة→إرسال؛ تونس فتح→مسح→10/10→إكمال→تقرير؛ A+B بالتوازي؛ بيانات AYBTEST-/AYPTEST- قابلة للتصفير؛ APK نهائي + روابط + تقرير التسليم §25).
