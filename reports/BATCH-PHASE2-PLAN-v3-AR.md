...
**الحالة:** العقد + الباركود + الخدمات + **Admin UI — كلها مكتملة وCI أخضر** — الفرع `feature/ayrovi-batch` @ `6999a48`
...
2. ~~Admin UI~~ ✅ **مكتملة** (`6999a48`) — لوحة `/admin/batches`: فلترة بالحالة، بطاقات الدفعة + وسام needsReview على عملاء العامل، تفصيل الوحدات (AYP + الأصل حرفيًا + MANUAL موسوم)، أزرار = مصفوفة pure تساوي آلة الحالات (9 اختبارات vitest)، قبول/إرسال (مفتاح idempotency لكل نقرة)/إلغاء بسبب إلزامي، وطباعة ملصق الدفعة QR بـ @zxing الموجودة — بلا أي تبعية جديدة، بمفردات os-*/ac-* الموحدة. 142/142 اختبار، tsc 0، build ✓.
...
**قاعدة البوابة:** لا انتقال لشريحة جديدة باختبارات حمراء — الوضع الآن أخضر بالكامل على الفرع. **التالي: تطبيق العامل** (إنشاء عميل → دفعة → وحدات AYP → تصدير/طباعة الملصقات → إرسال).
